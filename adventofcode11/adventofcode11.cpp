...
void main()
{
	...
	while (running)
	{
		if (painted.count(u.id) == 0)
			painted[u.id] = 0;

		program.setInput(painted[u.id]);
		...
		int change = output == 1 ? 1 : 3;
		facing = (facing + change) % 4;
